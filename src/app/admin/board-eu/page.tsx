...
export default function AdminBoardEUPage() {
  ...
  const loadData = async () => {
    const res = await fetch('/api/admin/board-eu')
  ...
  const addMember = async () => {
    const { data: { session } } = await supabase.auth.getSession()
    await fetch('/api/admin/board-eu', { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${session?.access_token || ''}` }, body: JSON.stringify({ entity: 'member', name: 'New Member', is_active: true, links: [], display_order: (members?.length || 0) + 1 }) })
    await loadData()
  }

  const updateMember = async (member: BoardMember) => {
    const { data: { session } } = await supabase.auth.getSession()
    await fetch('/api/admin/board-eu', { method: 'PUT', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${session?.access_token || ''}` }, body: JSON.stringify({ entity: 'member', ...member }) })
  }
