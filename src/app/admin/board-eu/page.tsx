...
export default function AdminBoardEUPage() {
  ...
  const loadData = async () => {
    setError(null)
    const { data: { session } } = await supabase.auth.getSession()
    const res = await fetch('/api/admin/board-eu', { headers: { Authorization: `Bearer ${session?.access_token || ''}` } })
  ...
  const addTab = async () => {
    const { data: { session } } = await supabase.auth.getSession()
    await fetch('/api/admin/board-eu', { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${session?.access_token || ''}` }, body: JSON.stringify({ entity: 'tab', name: 'New Tab', content: '[]', display_order: (tabs?.length || 0) + 1 }) })
    await loadData()
  }

  const updateTab = async (tab: BoardTab) => {
    const { data: { session } } = await supabase.auth.getSession()
    await fetch('/api/admin/board-eu', { method: 'PUT', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${session?.access_token || ''}` }, body: JSON.stringify({ entity: 'tab', id: tab.id, name: tab.name, content: tab.content, display_order: tab.display_order }) })
  }

  const deleteTab = async (id: string) => {
    const { data: { session } } = await supabase.auth.getSession()
    await fetch(`/api/admin/board-eu?entity=tab&id=${id}`, { method: 'DELETE', headers: { Authorization: `Bearer ${session?.access_token || ''}` } })
    await loadData()
  }

  const addMember = async () => {
    const { data: { session } } = await supabase.auth.getSession()
    const res = await fetch('/api/admin/board-eu', { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${session?.access_token || ''}` }, body: JSON.stringify({ entity: 'member', name: 'New Member', is_active: true, links: [], display_order: (members?.length || 0) + 1 }) })
    const json = await res.json().catch(() => ({}))
  ...
  const updateMember = async (member: BoardMember) => {
    setError(null)
    const { data: { session } } = await supabase.auth.getSession()
    const res = await fetch('/api/admin/board-eu', { method: 'PUT', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${session?.access_token || ''}` }, body: JSON.stringify({ entity: 'member', ...member }) })
    if (!res.ok) {
  ...
  const deleteMember = async (id: string) => {
    const { data: { session } } = await supabase.auth.getSession()
    await fetch(`/api/admin/board-eu?entity=member&id=${id}`, { method: 'DELETE', headers: { Authorization: `Bearer ${session?.access_token || ''}` } })
    await loadData()
  }

  const handleImageUpload = async (file: File, onUrl: (url: string, storagePath: string) => void) => {
    const { data: { session } } = await supabase.auth.getSession()
    const signRes = await fetch('/api/admin/upload/signed', { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${session?.access_token || ''}` }, body: JSON.stringify({ folder: 'board-eu/members', filename: file.name }) })
    ...
    if (!putRes.ok) return
    const readRes = await fetch('/api/admin/upload/signed', { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${session?.access_token || ''}` }, body: JSON.stringify({ action: 'sign-read', path }) })
    ...
    return (
      <div className="min-h-screen bg-white">
        ...
        <div className="max-w-3xl mx-auto px-4 py-20">
          ...
          <p className="text-[#757575]">You must be a super admin to manage the EU board.</p>
        </div>
        <Footer />
      </div>
    )
    ...
    <div className="min-h-screen bg-white">
      ...
      <main className="py-10">
        <div className="max-w-6xl mx-auto px-4 space-y-10">
          <div>
            <h1 className="font-['Barlow'] font-medium text-3xl mb-2">Manage Board EU</h1>
            <p className="text-[#757575]">Edit EU board tabs and members.</p>
          ...
          <section>
            ...
            <div className="grid gap-4">
              ...
              {tabs.length === 0 && <div className="text-[#757575]">No tabs created.</div>}
            </div>
          </section>
          ...
          <section>
            ...
            <div className="grid gap-4">
              ...
              {members.length === 0 && <div className="text-[#757575]">No members added.</div>}
            </div>
          </section>
        </div>
      </main>
